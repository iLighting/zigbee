...
static uint8 set_default_items(struct item_t **head_p)
{
  ...
  item_append(*head_p, 2, "yue du chang jing");
  item_append(*head_p, 3, "ying yuan chang jing");
  item_append(*head_p, 4, "wen xin chang jing");
  item_append(*head_p, 5, "quan liang chang jing");
  item_append(*head_p, 6, "guan deng");
  item_append(*head_p, 7, "ye deng chang jing");
  return 1;
}
...
uint16 asr_event_loop(uint8 task_id, uint16 events)
{
  ...
  {
    unsigned char res0 = ld3320_get_asr_result(0);
#ifdef ASR_DEBUG
    printf("asr evt: find = %d\n", (int)res0);
#endif
    asr_dispatch_one_msg(ASR_MSG_FIND, res0);
