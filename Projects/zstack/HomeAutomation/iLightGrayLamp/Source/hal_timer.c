...
#define T1CCTL_RFIRQ 0x80      /* Interrupt Mask for RFIRQ*/
...
#define HAL_TIMER1_OPMODE_UPDOWN 0x03  /* Modulo Mode, Count up from 0 to CompareValue and down to 0 */
...
/* Prescale settings */
#define HAL_TIMER1_16_PRESCALE HAL_TIMER1_16_TC_DIV8
#define HAL_TIMER1_16_PRESCALE_VAL 8
...
#define TCHN_T1CCTL &(XX_T1CCTL0)
#define TCHN_T1CCL &(XX_T1CC0L)
#define TCHN_T1CCH &(XX_T1CC0H)
...
uint16 halTimer1SetPeriod(uint32 timePerTick);
...
uint16 halTimer1SetPeriod(uint32 timePerTick)
{
  ...
  count = (uint16)((timePerTick * halTimerRecord[HW_TIMER_1].clock) / halTimerRecord[HW_TIMER_1].prescaleVal) / 2;
  halTimer1SetChannelCount(HAL_T1_CH0, count);
  return count;
}
